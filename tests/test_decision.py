...
def test_decision_paths_allow_then_block_on_insufficient():
    # Start with balance 300 (seeded lazily). Spend 250 allow, then try 100 block.
    p1 = {
    ...
    assert r2.json()["decision"] in ("block","review")  # usually block due to insufficient funds
